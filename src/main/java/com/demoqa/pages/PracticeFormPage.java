...
public class PracticeFormPage extends BasePage{
    ...
    WebElement dateOfBirthDayInput;
    public PracticeFormPage enterBirthDay(String date) {
        //focus on the field
        clickWithJS(dateOfBirthDayInput,0, 200);
        //select os
        String os = System.getProperty("os.name");
        System.out.println("My OS is " + os);
        // select on the keyBoard

        if (os.startsWith("Mac")){
            dateOfBirthDayInput.sendKeys(Keys.COMMAND,"a");
        } else {
            dateOfBirthDayInput.sendKeys(Keys.CONTROL,"a");
        }
        //enter text
        dateOfBirthDayInput.sendKeys(date);
        //press on keyboard the kay ENTER
        dateOfBirthDayInput.sendKeys(Keys.ENTER);
    ...
    public PracticeFormPage verifySuccessTitle(String titil) {
        Assert.assertTrue(shouldHaveText(successTitle,titil,10));
        return this;
    }
}
